...
TMDB_API_KEY = '8265bd1679663a7ea12ac168da84d2e8&language=en-US' #Poster için
...
def recommend(user_id, num_recommendations=10):
    ...
    if not user_history:
        return [{'title': "Tracking history not found.", 'movie_id': None, 'poster_url': None}]
    ...
    if similar_user_movies.empty:
        ...
        if last_watched_movie_id not in movies['movie_id'].values:
            return [{'title': f"Last viewed movie (movie_id: {last_watched_movie_id}) not found in the dataset.",
...
def add_to_history():
    ...
    if not movie_id:
        return render_template('movie_recommender.html', recommendations=[],
                               movies=movies['title'].tolist(), selected_movie=movie,
